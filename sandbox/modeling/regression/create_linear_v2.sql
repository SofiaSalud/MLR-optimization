CREATE OR REPLACE MODEL `sofia-data-305018.mlr_optimization.linear_reg_v2`
TRANSFORM(
  ...
  --ML.FEATURE_CROSS(STRUCT(member_address_neighborhood, doctor_neighborhood, provider_address_neighborhood)) AS neighborhood_cross,
  --ML.FEATURE_CROSS(STRUCT(doctor_name, doctor_descriptor)) AS doctor_cross,
  --ML.FEATURE_CROSS(STRUCT(provider_name, provider_category_name)) AS provider_cross,
  --ML.FEATURE_CROSS(STRUCT(service_type, cpt, diagnosis)) AS service_cross,
  subtotal
)
OPTIONS(
